...
export class CameraProvider implements MotionProvider {
  ...
  private createWorker(): Worker {
    return new Worker('/camera-worker.js?v=3')
  }
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        worker.onerror = (event) => {
          ...
          reject(new Error(event.message || 'Camera worker failed to load'))
        }
        ...
        worker.onmessage = (event) => {
          if (event.data.type === 'ready') {
            ...
            clearTimeout(timeout)
            reject(new Error(event.data.message || 'Camera worker init failed'))
          }
        }
      ...
      this.worker.onmessage = (event) => {
        if (event.data.type === 'result') {
          ...
          this.detectInFlight = false
          this.setStatus({ state: 'error', message: event.data.message || 'Hand detection failed' })
        }
      }
  ...
  private loop = (): void => {
    ...
    if (!this.video || !this.worker || this.detectInFlight || this.video.readyState < 2) return
