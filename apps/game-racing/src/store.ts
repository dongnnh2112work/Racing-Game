...
const exclusiveBooleans = ['help', 'leaderboard', 'pickcolor', 'settings'] as const
...
export interface IState extends BaseState {
  actions: Actions
  activeInputSource: 'wheel' | 'hands' | 'keyboard'
  api: PublicApi | null
  ...
  racingInputConfig: RacingInputConfig
...
const useStoreImpl = create<IState>((set: SetState<IState>, get: GetState<IState>) => {
  ...
  return {
    ...
    color: '#FFFF00',
    controls,
    racingInputConfig: { ...defaultRacingInputConfig },
